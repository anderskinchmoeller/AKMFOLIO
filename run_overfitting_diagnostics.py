...
Repository adapters below use HRPConfig for execution, fresh allocators for
CPCV splits, and CSV artifacts from write_robustness_report. --quick reduces
splits and bootstrap draws; it still performs chronological model training.
"""
    ...
    for attr in ("diagnostics", "kelly_diagnostics", "last_kelly_diagnostics", "rebalance_diagnostics"):
        if hasattr(result, attr):
            ...
            if isinstance(d, pd.DataFrame):
                return d.set_index(pd.to_datetime(d["date"]), drop=False) if "date" in d else d
    return None


def _extract_cpcv_path_sharpes(result: Any) -> list:
    if isinstance(result, dict):
        return [_sharpe(_extract_returns_series(path)) for path in result.values()]
...
def _build_allocator(config: Any, balanced_pit: pd.DataFrame):
    if balanced_pit is None:
        raise ValueError("Retail Alpha ML MPC requires a PIT universe; provide --pit.")
    Allocator = getattr(ALLOCATOR_MODULE, "RetailAlphaMLMPCAllocator")
    return Allocator(balanced_pit=balanced_pit, config=config)


def _score_returns(ctx, result):
    returns = _extract_returns_series(result)
    if ctx.evaluation_start:
        returns = returns.loc[returns.index >= pd.Timestamp(ctx.evaluation_start)]
    if ctx.evaluation_end:
        returns = returns.loc[returns.index <= pd.Timestamp(ctx.evaluation_end)]
    return returns


def _run_walk_forward(ctx, allocator):
    return ENGINE_MODULE.run_walk_forward(ctx.returns, allocator, ctx.backtest_config)


class _EqualWeightAllocator:
    config = SimpleNamespace(min_weight=0.0, max_weight=0.2)
...
def check_walk_forward_oos(ctx: Context, store: dict) -> CheckResult:
    ...
    result = _run_walk_forward(ctx, ctx.allocator)
    r = _score_returns(ctx, result)
    if len(r) < 2:
        raise ValueError("Fewer than two scored returns; provide more history before the evaluation window.")
    ...
    if diagnostics is not None:
        diagnostics = diagnostics.loc[diagnostics.index.isin(r.index)]
...
def check_kelly_buffer_filled(ctx: Context, store: dict) -> CheckResult:
    ...
    baseline_allocator = _build_allocator(baseline_config, ctx.pit)
    result = _run_walk_forward(ctx, baseline_allocator)
    baseline_r = _score_returns(ctx, result)
...
def check_cpcv(ctx: Context, store: dict) -> CheckResult:
    ...
    score_index = ctx.returns.index
    if ctx.evaluation_start:
        score_index = score_index[score_index >= pd.Timestamp(ctx.evaluation_start)]
    group_weeks = len(score_index) // ctx.cpcv_splits
    if group_weeks < 1:
        raise ValueError("Evaluation window is too short for the requested CPCV groups.")
    cfg = SimpleNamespace(**asdict(ctx.backtest_config))
    cfg.cpcv = CPCVConfig(
        n_groups=ctx.cpcv_splits, test_groups=min(2, ctx.cpcv_splits - 1),
        group_weeks=group_weeks, purge_weeks=ctx.cpcv_embargo_weeks,
        embargo_weeks=ctx.cpcv_embargo_weeks,
    )
    cfg.cpcv_refit_per_split = True
    result = ENGINE_MODULE.run_cpcv_backtest(
        ctx.returns, lambda train: _build_allocator(ctx.config, ctx.pit), cfg,
    )
...
def check_block_bootstrap(ctx: Context, store: dict) -> CheckResult:
    ...
    if "walk_forward_result" not in store:
        return CheckResult(name="block-bootstrap", status="SKIPPED",
                           headline="walk-forward-oos did not complete; no results to bootstrap")
    report_dir = ctx.output_dir / "robustness"
    benchmark = _run_walk_forward(ctx, _EqualWeightAllocator())
    ROBUSTNESS_MODULE.write_robustness_report(
        {ctx.model_name: store["walk_forward_result"], "equal_weight": benchmark},
        report_dir, benchmark="equal_weight", evaluation_start=ctx.evaluation_start,
        samples=ctx.n_bootstrap, block_weeks=ctx.block_length_weeks,
    )
    summary = pd.read_csv(report_dir / "summary.csv").set_index("model")
    report = summary.loc[ctx.model_name].dropna().to_dict()
    if report.get("status") != "reported":
        raise ValueError(f"Robustness report: {report.get('status')}")
    intervals = pd.read_csv(report_dir / f"{ctx.model_name}_bootstrap_intervals.csv", index_col=[0, 1])
    report["bootstrap_sharpe_ci"] = [
        float(intervals.loc[("strategy", q), "sharpe"]) for q in (0.025, 0.975)
    ]
    costs = pd.read_csv(report_dir / f"{ctx.model_name}_fixed_trade_cost_stress.csv")
    report["cost_stress"] = costs.set_index("cost_multiplier")["sharpe"].to_dict()
    store["robustness_report"] = report
    ci = None
...
def check_repair_frequency(ctx: Context, store: dict) -> CheckResult:
    ...
    if repaired is None and relaxed is None:
        return CheckResult(name="repair-frequency", status="SKIPPED",
                           headline="No optimizer repair diagnostics available")
    status, notes = "PASS", []
...
def check_placebo(ctx: Context, store: dict) -> CheckResult:
    ...
    placebo_allocator = _build_allocator(placebo_config, ctx.pit)
    result = _run_walk_forward(ctx, placebo_allocator)
    placebo_r = _score_returns(ctx, result)
...
def check_significance(ctx: Context, store: dict, trial_counts) -> CheckResult:
    ...
    if source is None and module is not None and hasattr(module, "sharpe_significance"):
        psr_fn = lambda series, **kw: module.sharpe_significance(series)["probabilistic_sharpe_ratio"]
        dsr_fn = lambda series, K=1, **kw: module.sharpe_significance(series, number_of_trials=K)["deflated_sharpe_ratio"]
        source = f"repo module ({used_path}.sharpe_significance)"
...
def parse_args(argv=None):
    ...
    if args.cpcv_splits < 2 or args.rebalance_every_weeks < 1:
        p.error("CPCV splits must be >= 2 and rebalance interval must be >= 1")
    if args.quick:
...
def main(argv=None):
    ...
    if args.data_start:
        returns = returns.loc[returns.index >= pd.Timestamp(args.data_start)]
    if args.evaluation_end:
        returns = returns.loc[returns.index <= pd.Timestamp(args.evaluation_end)]
    from akm_hrp.config import HRPConfig
    backtest_config = replace(
        HRPConfig(), pit_universe_file=args.pit, strict_pit_universe=bool(args.pit),
    ...
    allocator = _build_allocator(config, pit)

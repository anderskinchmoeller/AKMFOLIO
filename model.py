...
Mirrors the design used for the retail-capacity MPC model: two regularized
online ridge regressions, fit at different decay rates, learn a small fixed
set of economically motivated interaction features derived from the parent
allocator's five signals. A stock receives an ML score only when the fast
and slow models agree on its sign. All of it plugs into the existing
`_build_signal_panel` hook, so the parent's factor risk model, multi-period
optimizer, execution-cost model, and online IC-weighting machinery are left
untouched -- the ML signal is learned and weighted through exactly the same
Bayesian IC-shrinkage process as the other five signals.
"""
...
from akm_hrp.allocators.dynamic_barra_alpha import _rank_normal
...
class RetailAlphaMLMPCConfig(RetailAlphaMPCConfig):
    """Predeclared regularization and learning-speed controls for the ML sleeve.

    Cross-sections, rather than individual securities, receive equal weight in
    the sufficient statistics. This prevents large universes from swamping the
    history and makes results less sensitive to membership changes.
    """
    ...
    # A sixth competing signal is present now; keep any single signal
    # (including this one) from dominating the blend.

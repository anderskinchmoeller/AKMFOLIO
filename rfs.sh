...
#
# --retail-alpha-ml-max-weight 1.0 REMOVES the 3% per-name cap that
# RetailAlphaMPCConfig.max_weight imposes by default. Concentration is then
# bounded only by the sector caps, the +/-0.25 style-exposure caps and the
# 60-name trim. Expect slower solves and more repair passes: the optimizer's
# per-name box goes from [0, 0.03] to [0.01, 1.0]. Set a number between the
# two (0.05, 0.10) for a softer cap.
#
...
echo "cadence: 13w, budget: $BUDGET, min weight: 1%, max weight: NO CAP, turnover cap: 2.0"
"$PYTHON" -u -m akm_hrp.cli.compare_models \
  ...
  --dashboard-title "13w, budget=$BUDGET, 1% floor, NO 3% cap, turnover cap 2.0: retail_alpha_ml_mpc vs Equal Weight" \
  "$@" 2>&1 | tee "$OUT/run.log"
